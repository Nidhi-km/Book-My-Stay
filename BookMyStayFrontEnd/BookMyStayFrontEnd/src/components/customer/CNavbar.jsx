...
import { Menu, X, Home, LogIn, UserPlus, LogOut, User, BookOpen } from "lucide-react";
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-white shadow-md border-b border-gray-100">
      <div className="max-w-7xl mx-auto flex justify-between items-center px-5 py-4">
        ...
        <div className="hidden md:flex items-center gap-8 text-[#0A2647] font-medium">
          <Link to="/" className="hover:text-[#F5C518] flex items-center gap-1 transition-all duration-200">
            <Home size={18}/> Home
          </Link>

          {!isLoggedIn ? (
            <>
              <Link to="/customer/signin" className="hover:text-[#F5C518] flex items-center gap-1 transition-all duration-200">
                <LogIn size={18}/> Sign In
              </Link>
            </>
          ) : (
            <>
              <Link to="/customer/mybookings" className="hover:text-[#F5C518] flex items-center gap-1 transition-all duration-200">
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-200 flex flex-col items-center gap-3 py-3 text-[#0A2647] font-medium">
          <Link to="/" onClick={() => setIsOpen(false)} className="hover:text-[#F5C518] flex items-center gap-1">
            <Home size={18}/> Home
          </Link>

          {!isLoggedIn ? (
            <>
              <Link to="/customer/signin" onClick={() => setIsOpen(false)} className="hover:text-[#F5C518] flex items-center gap-1">
                <LogIn size={18}/> Sign In
              </Link>
              <Link to="/customer/signup" onClick={() => setIsOpen(false)} className="hover:text-[#F5C518] flex items-center gap-1">
                <UserPlus size={18}/> Sign Up
              </Link>
            </>
          ) : (
            <>
              <Link to="/customer/mybookings" onClick={() => setIsOpen(false)} className="hover:text-[#F5C518] flex items-center gap-1">
                <BookOpen size={18}/> My Bookings
              </Link>
              <Link to="/customer/profile" onClick={() => setIsOpen(false)} className="hover:text-[#F5C518] flex items-center gap-1">
